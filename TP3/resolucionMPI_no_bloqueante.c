...
int main(int argc, char *argv[]){
    ...
    multiplacionMatricesBloque(C, D, res2, blockSize, n, stripSize);
    ...
    if (rank == MASTER) {
        ...
        printf("El resultado en R[0] es: %f\n", R[0]);

        for (int i = 0; i < n * n; i++) {
            if (R[i] != n) {
                printf("Error en el resultado, el valor es: %f\n", R[i]);
                break;
            }
        }
