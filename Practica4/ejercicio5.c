...
int main(int argc, char *argv[]){
    ...
    if (rank == MASTER) {
        ...
        res1 = (double*) malloc(sizeof(double)*n*stripSize);
        res2 = (double*) malloc(sizeof(double)*n*stripSize);
        res3 = (double*) malloc(sizeof(double)*n*stripSize);
    }
    else  {
    ...
    MPI_Scatter(R, n*stripSize, MPI_DOUBLE, R, n*stripSize, MPI_DOUBLE, MASTER, MPI_COMM_WORLD);
    ...
    if (rank==MASTER) {
		...
		if(check){
			...
			printf("Multiplicacion de funcion resultado erroneo\n");
		}
